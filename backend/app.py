...
@app.route('/backend/request_chat', methods=["POST"])
@jwt_required
def request_chat():
...
@app.route("/backend/pusher/auth", methods=['POST'])
@jwt_required
...
@app.route("/backend/send_message", methods=["POST"])
@jwt_required
def send_message():
    request_data = request.get_json()
    from_user = request_data.get('from_user', '')
...
@app.route('/backend/get_message/<channel_id>')
@jwt_required
